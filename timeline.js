  ...
  UDNZTimelineJS = function(options) {
    ...
    this.__drawNodes = function(data) {
      for (var idx in data.nodes) {
        ...
        var pTo = {
          x:
            (parseInt(this.params.container.width) / 100) *
    ...
    this.__drawBranches = function(nodes, branchFrom, branchIdx) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        var pToChild = {
          ...
          y:
            parseInt(this.params.container.height) / 2 + parseInt(item.offsetY),
          idx: branchFrom.idx
        };
        ...
        this.__drawHotDot(
          item,
          this.params.dots._required.css.branch,
          pToChild
        );
      ...
      // build lineStyle
      {
        ...
        if (
          ...
          if (!!node.lines.type && node.lines.type === "dotted") {
            lineStyle +=
              "stroke-dasharray:" + node.lines.stroke_dasharray + ";";
    ...
    this.__drawBezier = function(args) {
      ...
      d +=
        ...
        " ";
      d +=
        "L" + to.x + " " + parseInt(to.y + this.params.lines.width / 2) + " ";
      d +=
        "Q" +
        parseInt(from.x + Math.abs(offset)) +
        " " +
      ...
      if (!!nodeId) {
        ...
        if (!!node) {
          ...
          // set position
          {
            ...
            var top =
              offsetY - offsetFromBoard + offsetFromLine + offsetFromDot;
